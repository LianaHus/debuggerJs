var vmTrace = [{
      ...
      steps: 32,
      storage: {
      	"@ 0 (0x0)": '186403985281000846090828659806414249962070741508(0x20a6a4cf…)'
      }
      }, {
      gas: '707523',
      gascost: '3',
...
function init(id, memTable, stackTable, storageTable){
    $.each(vmTrace, function(key, value) {   
        $(id)
            .append($("<option></option>")
            .attr("value", vmTrace[key].steps)
            .text(vmTrace[key].steps + "    " + vmTrace[key].instname)); 
    });
    $( id ).change(function() {
    	$( id + " option:selected" ).each(function(key, value) {
	    	...
	    	// Stack
	    	$(stackTable).html("");
	    	var stackContent = vmTrace[$( this ).attr("value")].stack;
	    	// TODO reverse the order for ex use $(stackContent).reverse();
    		...
    		// Storage
    		$(storageTable).html("")
    		var storageContent = vmTrace[$( this ).attr("value")].storage;
    		if (storageContent) {
